...
수정버튼 클릭 후 수정을 하지않고 수정을 누를시 value 를 못받아오는 문제를 해결하기 위해, useInputs 를 사용할 때 기본값을 빈값으로 만들어 버리던 걸 아래와 같이 수정했다. (submit 후도 같은 방식으로 수정)
취소버튼 클릭시에 기본값을 빈값으로 만들던 것도 아래와 같은 방식으로 수정했다.
